...
func (t *TCCChecker) IsConfirm() (bool, error) {
	log.Print("tcc checker is confirm")
	row := 0
	err := t.db.QueryRow("select count(1) from local_confirm_log where tx_no = ?", t.tid).Scan(&row)
	return row > 0, err
}
